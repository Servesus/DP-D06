...
public class MessageServiceTest extends AbstractTest {
	...
	public void testDeleteMessage() {
		...
		spam.setSubject("Hola");
		final Message savedS = this.messageService.save(spam);
		this.messageService.deleteMessage(savedS);
		final List<Box> boxes = new ArrayList<Box>(saved.getBoxes());
		Assert.isTrue(boxes.get(2).getMessages().contains(savedS));
		this.messageService.deleteMessage(savedS);
		Assert.isNull(this.messageService.findOne(savedS.getId()));
	...
	public void testMoveMessage() {
		...
		final HandyWorker sender = saved;
		final List<Box> senderBoxes = (List<Box>) sender.getBoxes();
		this.messageService.moveMessage(savedM, senderBoxes.get(3));
		final List<Box> boxes = (List<Box>) sender.getBoxes();
		final Box boxR = boxes.get(3);
		final Box boxS = boxes.get(1);
		Assert.isTrue(boxS.getMessages().isEmpty() && !boxR.getMessages().isEmpty());
	}
